...
async fn reveal_recheck_separates_kept_promises_from_debt() {
    ...
    let (withheld, dated, due, checked, revealed, by_field) =
        db.reveal_recheck(now, 20_000).await.expect("recheck");
    assert_eq!(withheld, 4, "every FieldsPrivacy section counts");
    assert_eq!(dated, 3, "three carry a BT-198 date");
    assert_eq!(due, 2, "two dates have passed");
    assert_eq!(checked, 2);
    assert_eq!(revealed, 1, "cri-num vanished from the later version");
    assert_eq!(checked - revealed, 1, "win-nam is the standing reveal debt");
    assert_eq!(by_field.len(), 2, "the due breakdown names both fields");
    assert!(by_field.iter().any(|(f, n)| f == "cri-num" && *n == 1));
    assert!(by_field.iter().any(|(f, n)| f == "win-nam" && *n == 1));
}

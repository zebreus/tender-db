...
fn quarantine_reason_explained(reason: &str) -> &'static str {
    match reason {
        ...
        "unmapped-era" => "A real notice from an era whose parser does not exist yet — the 2008 OPOCE INTERNAL_OJS export (~28k notices, issue 41).",
        "missing-publication-id" => "The member carried no publication id of its own, so it is not a distinct notice.",
        "unparsable-xml" => "Genuinely malformed XML that could not be parsed (the DTD-bearing INTERNAL_OJS bucket is now the tracked 'unmapped-era', issue 36).",

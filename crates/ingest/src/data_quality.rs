...
/// window is an index range.
///
/// **The window is TWO-SIDED, and the first version of this query was not.**
/// Shipped as `9073eca` with only the floor, it cost ~67 minutes on prod and was
/// reverted (`5b0eb4c`). The result was right and the work was not: the join to
/// `h.p = n.profile` discards rows from other profiles, but only AFTER the range
/// scan has read them, so every profile scanned from its own floor to the end of
/// the corpus. Twenty-four heavily overlapping one-sided scans instead of
/// twenty-four bounded windows, worst for the oldest profile.
///
/// Measured on prod before re-landing, on `notice_texts` at r208's head:
///
///   WHERE notice_id BETWEEN max-100k AND max    3.5 s   (1,333,888 rows)
///   WHERE notice_id > max-100k                  >10 s   (10 s cap, timed out)
///
/// `AND x.notice_id <= h.max_id` is the whole fix. It changes no result — an id
/// above a profile's maximum cannot belong to that profile — and it is what
/// makes the predicate a bounded index range instead of an open-ended one.
...
mod tests {
    ...
    fn the_per_profile_window_sees_an_era_the_corpus_window_cannot() {
        ...
        // The upper bound is not decoration: without it every profile scans from
        // its own floor to the END of the corpus, because the profile join
        // discards the other eras' rows only AFTER the range scan has read them.
        // That shipped once (9073eca) and cost ~67 minutes; measured on prod, the
        // bounded form of the same range is 3.5 s where the open-ended one
        // exceeds the 10 s read cap.
        assert!(
            sql.contains("AND x.notice_id <= h.max_id"),
            "the window is TWO-SIDED, or it is not a window:\n{sql}"
        );
        assert!(

# Rubric v3 — same-name cross-border organization cases (issues 311/314/355)

You are reviewing organization rows from a public-procurement database (EU TED + national
sources). Each CASE is a small set of `organizations` rows (usually a pair) that share one
normalized name but stand under DIFFERENT country codes. Your job, per case and per row: decide
...
Output exactly one entry per case root, with `country_moves` empty unless the verdict is
`wrong-country`. Rationale: two or three sentences naming the evidence fields you used.

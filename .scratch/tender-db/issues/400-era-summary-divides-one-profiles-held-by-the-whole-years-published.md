...
Status: **DONE 2026-09-18** — the body recorded RESOLVED-VERIFIED on 2026-09-16 (rev `c40cccc`: every era row divides `year_held` by the years' published — `eforms-sdk-1.5` 0.00 % → 100.00 %, `internal-ojs` 7.94 % → 100.14 %, r208/r209 100.00 %†) and ended "Closing."; this line never followed (issue 412, mechanism (b)). The one carve-out its own addendum names — the era summary's partial-year denominator still carries no ‡ — stays recorded there as NOT done, one glyph beyond the unit. Was: ready-for-agent — filed 2026-09-16 from issue 396 unit 1's carve-out ("Adjacent, recorded but NOT part of this unit … either fixed with this or filed as its own issue against 229's shape"); 396's own fix shipped without touching it, so this is the "filed as its own issue" branch, taken because the rendering question below is a real design choice that wants its own measurement, not a rider on a copy fix

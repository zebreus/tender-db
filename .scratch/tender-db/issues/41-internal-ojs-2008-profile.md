...
2. A `_SUM` alias map so the r209 rules match the summary elements. **It must be
   an explicit, checked-in table** (`X_SUM → X`), generated by the sweep and
   committed — NOT a runtime strip-the-suffix rule (lead, 2026-07-21). Rationale:
   if any `_SUM` element ever diverges semantically from its base, an explicit
   table makes it a visible diff line and the bijection test pins it; a suffix
   rule would hide the drift silently. The sweep also flags any `_SUM` whose base
   is NOT an r209 element (none in package 1 — confirm across the full sweep).
...
Mined 2026-07-21 from **6 May-2008 packages** spread across the window
(`2008085 2008089 2008093 2008097 2008101 2008105`), plus the r209-diff.
**Committed corpus artifacts (read these, don't re-mine):**
- `41-vocab-sweep.tsv` — every element (634), its count, how many of the 6
  packages it appears in, and its attributes.
- `41-sum-alias-table.tsv` — the explicit **77-entry `_SUM` → r209-base alias
  table** (nuance 1). Every base is an r209 element (`base_in_r209=1`); **0 drift
  across the sweep**. This is the checked-in table the profile normalises through
  and the bijection test pins — regenerate/extend it in the build, never a
  runtime suffix strip.
- `41-new-elements.tsv` — the **20 non-`_SUM` new elements** (envelope +
  backbone), stable across all 6 packages.

Sweep numbers: 634 elements, 74 heading families, 97 new (77 `_SUM` aliases + 20
envelope/backbone). Breadth mattered — one package had 65 aliases, six have 77
(12 rarer-heading aliases the single package missed), while the 20 envelope
elements were stable. The one-package delta lists further below are the subset;
the committed `.tsv`s are authoritative.

**2010 tail (nuance 2):** could not be located cheaply — a `member_path NOT LIKE
'2008%'` scan 408s (the `quarantine_reason` index from issue 37 is not deployed
until #6; the bucket's sorted max is `20080531_2008105` and the min is a stray
`03…` path, so the ~1,898 2010 members sit under a different prefix I could not
reach in 10 s). After deploy #6 makes reason-filtered scans fast, the build must
grab a 2010 tail package, check its DOCTYPE version string against R2.0.5, and
either fold it into this inventory or scope it out explicitly — default to
deferring (1,898 ≈ 0.3% of the ~28k notices) if the version differs.

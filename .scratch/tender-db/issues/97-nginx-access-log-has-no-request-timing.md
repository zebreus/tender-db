...
Status: DONE — LIVE ON PROD 2026-08-28 02:48 UTC (owner). The staged config
installed on a fresh classifier attempt (single-purpose commands; the 08-25
refusals did not recur): `cp` + `nginx -t` (pass) + `systemctl reload nginx`.
Verified on REAL traffic: an external `/v1/events` line logs `rt=2.199
urt=2.199`, a local probe `rt=0.000` — both timing fields live. Backup at
/etc/nginx/nginx.conf.bak-issue97 stands for rollback.
Kind: observability

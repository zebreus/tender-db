...
Status: **DONE 2026-09-18** — the body recorded RESOLVED-VERIFIED on 2026-09-16 (rev `c40cccc`: `ecb`/`eurostat` render as reference feeds with `rates through <date>` and no `fetch complete ✓` claim; `ted`/`doe`/`fts` byte-identical to before) and ended "Closing."; this line never followed (issue 412, mechanism (b)). Was: ready-for-agent — found 2026-09-16 by the hourly audit (step 3), on the live dashboard at rev `19010b8`. Fully specified; the only open question is which of the three renderings below to take, and that decision is the first unit.

...
**Cohort 3 (`over-wall-3-2026-09-04`, over-wall names 400–799 by rows, 596 →
404 rows each):** 343 `single`, 14 `generic` (`Conseil General`, `conseil
regional`, `HOPITAL LOCAL`, `AMMINISTRAZIONE REGIONALE`, `STAATLICHES
HOCHBAUAMT`, `ZARZAD DROG WOJEWODZKICH`, the Polish hospital class names, the
bare CROUS and comité consultatif names), 11 `non-name` (`nie dotyczy`, `as
above`, `néant`, `deserto`, `Ingen`, `M. le président du tribunal
administratif`, `Véase el Pliego…`, price labels, form pointers), 1 `platform`
(`www.tendersmart.com`), 31 `unclear` (bare multinationals — `Hitachi`, `GE
Healthcare`, `Orange`, `Sandoz`, `DELL`, `Terumo`, `Smith & Nephew` — and
`The Recruitment Agency`, `Central Procurement Directorate`, `CAIR LGL`).
168,166 rows admitted. The in-flight dry plan (job 670) was cancelled before
its plan was recorded and re-run as job 671 so one plan carries both cohorts.

**Cohorts 2+3 planned (job 671, 1,113 s):** 654 `verdict-single` groups /
428,933 rows; 82 `verdict-refused`; 56,956 still `over-wall`. Seventeen of the
671 posted singles were missing from the plan: their stored names contain a
line break, and the cohort file had replaced it with a space, so the verdict
landed on a name that does not exist. Re-posted with the exact stored strings
(`over-wall-2-3-fix-2026-09-04`, 18 names); a cohort file must carry the
census's `name` verbatim, never a display copy. Wet run job 672.

**Cohort 4 (`over-wall-4-2026-09-04`, over-wall names 800–1,199 by rows, 403 →
318 rows each):** 348 `single`, 12 `generic` (`Hotel de Ville`, `Service des
marchés publics`, `Ministry of Foreign Affairs`, `MINISTRY OF EDUCATION`,
`AZIENDA UNITA SANITARIA LOCALE`, `CENTRAL FINANCE AND CONTRACTS UNIT`, the
city-less `CENTRE HOSPITALIER UNIVERSITAIRE, DIRECTION …`), 13 `non-name`
(`Lots 1`, `lotto deserto`, `non attribué`, `entfällt`, `TACIS`, `Der er intet
mæglingsorgan i Danmark`, price labels), 27 `unclear` (`Tyco`, `Gambro`,
`Xerox`, `Mylan`, `Astrazeneca`, `Colas`, `Brake`, a person, a consortium
string). 124,333 rows admitted; folds with the 18 fixed names in the next
cycle.

**Cohort 5 (`over-wall-5-2026-09-04`, the remaining 758 listed over-wall names,
317 → 229 rows each):** 661 `single`, 27 `generic` (`POWIATOWY URZAD PRACY`,
`KOMENDA WOJEWODZKA POLICJI`, `URZAD MIEJSKI`, `MIEJSKI ZAKLAD KOMUNIKACJI`,
`GEMEENTEBESTUUR`, `AZIENDA OSPEDALIERA`, `UNIVERSITA DEGLI STUDI`, `CENTRE
HOSPITALIER GENERAL`, `Centre hospitalier intercommunal`, `Centre hospitalier
Louis Pasteur` (four of them), `OPHLM`, `TGI`, `Department of Health` (UK and
IE), `FLEET AND INDUSTRIAL SUPPLY CENTRE`, `DIRECTORATE OF CONTRACTING`, the
bare comité consultatif variants, the French naval/army directorates), 23
`non-name` (`Niet gegund`, `Non aggiudicato`, `Marché infructueux`, `siehe
oben`, `jak wyżej`, `jw`, `wie vor`, `Lots 2)`, `Appalto n`, a fax number,
price labels, VI.4.2 pointers, download-URL sentences), 47 `unclear` (bare
multinationals — `philips`, `amgen`, `jeol`, `depuy`, `Biomet`, `KPMG`, `Carl
Zeiss`, `GDF Suez`, `Sodexho`, `Lyreco`, `Bull` — and names two countries
share: `PROVINCIE LIMBURG` (NL/BE), `Stadt Burgdorf` (DE/CH), `MINISTERIE VAN
JUSTITIE`, `MINISTERE DE LA DEFENSE NATIONALE`, `SERVICES DU PREMIER MINISTRE`,
`YPOYRGEIO POLITISMOY` (GR/CY), `Mindef`, `GENDARMERIE`, `AZIENDA ENERGETICA
MUNICIPALE (AEM)` (Milan/Turin), `Azienda Ospedaliera San Paolo`, three-letter
acronyms `CEA`, `RSS`, `ESPO`). 176,467 rows admitted. Country-qualified
subsidiaries (`Becton Dickinson France`, `Siemens PLC`, `Roche, Lda`, `3M
España`) and legal-form names (`… Sp. z o.o.`, `… S.R.L.`) stay `single`, as
in the earlier cohorts.

**Job 672 (wet, cohorts 2+3): parity abort, nothing written.** `p0 parity
abort: plan has 1360 groups, dry-run recorded 654 (tolerance 50)`. The wet
run re-plans from the live verdict table, and between job 671's dry plan and
672's walk the 18-name fix, cohort 4 (348 singles) and — during the walk
itself, so only the names alphabetically after its position — part of cohort
5 landed. The guard did exactly what it is for. The rule the sequence
teaches: a verdict POST invalidates the standing dry plan; post cohorts,
THEN dry-run, THEN wet, and nothing in between. Issue 353's shape breakdown
rides the next dry run.

**Cohorts 2–5 folded (job 674, 1,430 s):** dry job 673 planned 1,680
`verdict-single` groups / 740,280 rows (tiers: over-wall 55,855 ·
verdict-refused 157 · verdict-single 1,680); the wet run held parity at
1,680 and removed 740,280 rows — 740,232 mentions, 772,354 parties and
148,812 winners repointed, 521,148 tenders touched, ~1,500 rows/s with
foreign keys off. Bounded reads after: `city of edinburgh council` 1,278 →
1 with its 1,278 mentions on the keep; `london borough of redbridge` 310 →
1; `powiatowy urzad pracy` (generic) still 315. Unit 5's listed backlog is
closed; the ~55,900 names below the listing are issue 353's.

With cohort 5 every over-wall name the census listed (cap 2,000; 2,176
over-wall entries) has a verdict: 2,158 recorded across cohorts 1–5 plus the
18-name fix. Cohorts 4+5 together admit 300,800 rows; they fold in the next
dry+wet cycle after job 672. Beyond the listing sit ~55,500 over-wall names
with ≤229 rows each (~2.4M rows); the next unit is either a census listing cap
above 5,000 (cheap: the heap is bounded by the cap) or a cut that skips names a
verdict cannot settle from the string alone.

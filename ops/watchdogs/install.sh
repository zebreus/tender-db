...
for s in tender-db-diskwatch.sh tender-db-jobwatch.sh; do
...
for u in \
    ...
    tender-db-jobwatch.service  tender-db-jobwatch.timer; do
    install -m 0644 -o root -g root "$here/$u" "$unitdir/$u"
    echo "installed $unitdir/$u"
done

systemctl daemon-reload
systemctl reset-failed tender-db-diskwatch.service tender-db-jobwatch.service 2>/dev/null || true

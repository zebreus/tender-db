...
| `tenders t … JOIN tender_versions v … JOIN notice_codes c ON c.notice_id = v.caused_by_notice_id AND c.field_id = 'TXT-NC' WHERE t.id BETWEEN a AND b` | `SCAN notice_codes AS c` — the equality on `field_id` (no index) made the planner drive from the corpus's largest table and seek `tender_versions_notice` per row; the id range on `t` never bounded anything, so a 2,000-tender slice ran past the cap exactly like a 100,000-tender one (2026-09-18, issue 417). And a capped query is NOT cancelled: it pins its worker to completion — two of them left `/v1/sql` answering `503 saturated` for 13.5 minutes | `+c.field_id = 'TXT-NC'` — the plan becomes PK seek on `t`, index seek on `v`, PK seek on `c` (read locally, issue 417). Read the plan locally BEFORE a join-bridged read, and after one 408 send nothing else until `SELECT 1` answers again |

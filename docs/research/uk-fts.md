...
**Mapped-or-ignored checklist (ADR-0004, added 2026-09-18).** The profile `fts:ocds-1.1` keeps its per-path disposition in `crates/ingest/src/fts/checklist.rs`: every OCDS path the parser has decided about, as `Mapped(<the eForms field it becomes>)` or `Ignored(<why>)`, and `disposition(path)` answers by the longest entry on a path boundary (a container covers its subtree; `tender.lots` does not cover `tender.lotsGroup`). It is pinned by `every_published_fts_path_is_mapped_or_ignored_on_record` (`crates/ingest/tests/fts.rs`), a census over every release in the test fixtures — 224 distinct paths over 14 releases on the day it landed, all disposed — so a new key the register starts publishing fails the suite the first time a fixture carries it, instead of being silently dropped the way `contracts[].value` was (issue 386). Entries whose reason starts `owed:` are content the profile does NOT fold yet and has said so: party address lines and contact points (BT-510/512/513, BT-502/503/506), lot options and renewals (BT-58), the procedure type, nature and direct-award justification (BT-105, BT-23, BT-136), the award period, the lot's maximum extension date, the contract and award periods (`tender_version_contracts` has no duration columns — 386 unit 2b's schema decision), and the bid statistic's `relatedLot`. Reclassifying an entry from `owed` to `Mapped` is the checklist's own record of a unit landing.
